	...
	public static void main(String args[])
	{
		...
		try {
			...
			myRepository = manager.getRepository("Homework6");
			   ...
			   try {
				  TreeMap<String,GeoPoint> tree = new TreeMap<String,GeoPoint>(); 
...
" } ";
				  ...
				  try {
					  int count = 0;
						...
						if(dist !=-1 && dist <=200)
						{
							objPoint.distance = dist;
							tree.put(objPoint.getUnivURI(),objPoint);
						}}
			              
			          Set<Entry<String,GeoPoint>> entrySet =tree.entrySet();   
			          Iterator<Entry<String,GeoPoint>> i = entrySet.iterator();
			          System.out.printf("%-90s\t%-60s\t%-50s\t%s\n", "University URI", "University Name", "University City", "Distance from UCLA");
			          ...
			          while(i.hasNext())
			          {
			        	  Entry<String,GeoPoint> entry = i.next();
			        	   GeoPoint g =  entry.getValue();
			        	   System.out.printf("%-90s\t%-60s\t%-50s\t%f\n", g.getUnivURI(), g.getUnivName(), g.getCity(), g.distance);
